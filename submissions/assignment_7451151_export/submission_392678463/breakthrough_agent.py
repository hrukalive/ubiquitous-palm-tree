...
def minimax_cutoff_search(game, state, d=3, eval_fn=None):
    ...
    def max_value(state, depth):
        ...
        expanded_nodes += 1
        value = -1 * np.inf
        # print(f"Max", game.actions(state), game.to_move(state))
        for action in game.actions(state):
            value2, action2 = min_value(game.result(state, action), depth + 1)
            if value2 > value:
    ...
    def min_value(state, depth):
        ...
        expanded_nodes += 1
        value = np.inf
        # print(f"Min",game.actions(state), game.to_move(state))
        for action in game.actions(state):
            value2, action2 = max_value(game.result(state, action), depth + 1)
            if value2 < value:
                value, move = value2, action
    ...
    best_action = None
    _, best_action = max_value(state, 1)
...
def alpha_beta_cutoff_search(game, state, d=4, eval_fn=None):
    ...
    def max_value(state, alpha, beta, depth):
        ...
        value = -1 * np.inf
        for action in game.actions(state):
            value2, action2 = min_value(game.result(state, action), alpha, beta, depth + 1)
            if value2 > value:
                value, move = value2, action
    ...
    def min_value(state, alpha, beta, depth):
        ...
        value = np.inf
        for action in game.actions(state):
            value2, action2 = max_value(game.result(state, action), alpha, beta, depth + 1)
            if value2 < value:
                value, move = value2, action
                beta = min(beta, value)
            if value <= alpha:
    ...
    best_move = None
    _, best_move = max_value(state, -1 * np.inf, np.inf, 1)
