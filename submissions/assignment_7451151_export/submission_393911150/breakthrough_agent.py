...
def minimax_cutoff_search(game, state, d=3, eval_fn=None):
    ...
    expanded_nodes += 1
    for a in game.actions(state):
        value = min_value(game.result(state, a), d - 1)
        if value > best_value:
            best_value = value
            best_action = a
...
def alpha_beta_cutoff_search(game, state, d=4, eval_fn=None):
    ...
    def max_value(state, depth, alpha, beta):
        ...
        if eval_fn:
            actions.sort(
                key=lambda a: eval_fn(game.result(state, a), player),
                reverse=True
            )

        for a in actions:
            v = max(v, min_value(game.result(state, a), depth - 1, alpha, beta))
            if v >= beta:
                return v
            alpha = max(alpha, v)
    ...
    def min_value(state, depth, alpha, beta):
        ...
        for a in actions:
            v = min(v, max_value(game.result(state, a), depth - 1, alpha, beta))
            if v <= alpha:
                return v
            beta = min(beta, v)
    ...
    for a in game.actions(state):
        value = min_value(game.result(state, a), d - 1, alpha, beta)
