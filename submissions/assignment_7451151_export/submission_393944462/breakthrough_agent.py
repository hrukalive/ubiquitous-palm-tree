...
def minimax_cutoff_search(game, state, d=3, eval_fn=None):
    ...
    def max_value(state, depth):
        ...
        for action in game.actions(state):
            ...
            value, _ = min_value(next_state, depth - 1)
            if value > max_eval:
                max_eval = value
                best_action = action
    ...
    def min_value(state, depth):
        ...
        for action in game.actions(state):
            next_state = game.result(state, action)
            value, _ = max_value(next_state, depth - 1)
            if value < min_eval:
                min_eval = value
                best_action = action
...
def alpha_beta_cutoff_search(game, state, d=4, eval_fn=None):
    ...
    def max_value(state, alpha, beta, depth):
        ...
        for action in game.actions(state):
            next_state = game.result(state, action)
            value, _ = min_value(next_state, alpha, beta, depth - 1)
            if value > max_eval:
                max_eval = value
                best_action = action
    ...
    def min_value(state, alpha, beta, depth):
        ...
        for action in game.actions(state):
            next_state = game.result(state, action)
            value, _ = max_value(next_state, alpha, beta, depth - 1)
            if value < min_eval:
                min_eval = value
                best_action = action
            beta = min(beta, value)
    ...
    _, best_move = max_value(state, float('-inf'), float('inf'), d)
    return best_move, expanded_nodes
