...
def main(white_agent, black_agent):
    ...
    while running:
                ...
                if 0 <= r < 8 and 0 <= c < 8 and human_turn:
                    ...
                    if not human_moved:
                        if (
                            ...
                            and state["to_move"] == "WHITE"
                            or state["board"][r][c] == "BLACK"
                            and state["to_move"] == "BLACK"
                        ):
                            selected = (r, c)
                            highlighted = [
                                m for m in game.actions(state) if m["from"] == selected
                            ]
                        else:
                            selected = None
                            highlighted = []
        ...
        winner = (
            ...
            else ("W" if state["to_move"] == "BLACK" else "B")
        )
        if winner is not None:
            game_over = True
        clock.tick(60)
