...
def minimax_cutoff_search(game, state, d=3, eval_fn=None):
    ...
    best_v = float('-inf')
    for action in game.actions(state):
        v = min_value(game.result(state, action), d - 1)
        if v > best_v:
            best_v = v
            best_action = action
...
def alpha_beta_cutoff_search(game, state, d=4, eval_fn=None):
    ...
    def max_value(state, alpha, beta, depth):
        ...
        v = float('-inf')
        for action in game.actions(state):
            v_child = min_value(game.result(state, action), alpha, beta, depth - 1)
            if v_child > v:
                v = v_child
                alpha = max(alpha, v)
            if v >= beta:
    ...
    def min_value(state, alpha, beta, depth):
        ...
        v = float('inf')
        for action in game.actions(state):
            v_child = max_value(game.result(state, action), alpha, beta, depth - 1)
            if v_child < v:
                v = v_child
                beta = min(beta, v)
            if v <= alpha:
    ...
    alpha = float('-inf')
    for action in game.actions(state):
        v = min_value(game.result(state, action), alpha, float('inf'), d - 1)
        if v > best_v:
            best_v = v
            best_action = action
            alpha = best_v
