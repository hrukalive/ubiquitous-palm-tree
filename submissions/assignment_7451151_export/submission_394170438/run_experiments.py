import json

from breakthrough import offensive_eval_1, defensive_eval_1
from breakthrough import offensive_eval_2, defensive_eval_2
from breakthrough import play_game
from breakthrough_agent import MinimaxAgent, AlphaBetaAgent


# THIS FILE IS FOR PERFORMING EXPERIMENTS ON BREAKTHROUGH GAME
#   _____                            _              _
#  |_   _|                          | |            | |
#    | |  _ __ ___  _ __   ___  _ __| |_ __ _ _ __ | |_
#    | | | '_ ` _ \| '_ \ / _ \| '__| __/ _` | '_ \| __|
#   _| |_| | | | | | |_) | (_) | |  | || (_| | | | | |_
#  |_____|_| |_| |_| .__/ \___/|_|   \__\__,_|_| |_|\__|
#                  | |
#                  |_|
# YOUR NAME: Maddox Neddo
# YOUR WPI ID: 997074876
# FINISH THE ASSIGNMENT IN `breakthrough.py` AND `breakthrough_agent.py`
#   After implementing breakthrough game, you may run GUI to check.
#   After implementing adversarial search and provided heuristics, you may
#       test against random agent.
#   Finally, perform the experiments required for the report here.
# REQUIRED PACKAGES: click, numpy, pygame, tqdm


##########################################################################
#  __   __                  ____          _         _   _
#  \ \ / /__  _   _ _ __   / ___|___   __| | ___   | | | | ___ _ __ ___
#   \ V / _ \| | | | '__| | |   / _ \ / _` |/ _ \  | |_| |/ _ \ '__/ _ \
#    | | (_) | |_| | |    | |__| (_) | (_| |  __/  |  _  |  __/ | |  __/
#    |_|\___/ \__,_|_|     \____\___/ \__,_|\___|  |_| |_|\___|_|  \___|
# Perform the necessary experiments here to generate data required by the report.

def main():
    white = AlphaBetaAgent("AlphaBeta Off1", depth=3, eval_fn=offensive_eval_2)
    black = AlphaBetaAgent("AlphaBeta Off1", depth=3, eval_fn=defensive_eval_2)
    results = play_game(white, black, 400, True, False)

    total_sum = 0
    for n in white.nodes_per_move:
        total_sum += n
    print("White total nodes expanded: ", total_sum)
    total_sum = 0
    for n in black.nodes_per_move:
        total_sum += n
    print("Black total nodes expanded: ", total_sum)
    print("White nodes per move: ", results['white_nodes_per_move'])
    print("Black nodes per move: ", results['black_nodes_per_move'])
    print("White average time per move: ", results['white_time_per_move'])
    print("Black average time per move: ", results['black_time_per_move'])
    print("Total moves: ", results['total_moves'])


if __name__ == '__main__':
    main()
