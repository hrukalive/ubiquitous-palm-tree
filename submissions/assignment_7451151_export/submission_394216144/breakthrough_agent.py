...
def minimax_cutoff_search(game, state, d=3, eval_fn=None):
    ...
    def max_value(state, depth):
        ...
        # Check if depth cutoff reached
        if depth == 0:
            return eval_fn(state, player) if eval_fn else 0
    ...
    # Root level: find the best action for the current player
    best_action = None
...
def alpha_beta_cutoff_search(game, state, d=4, eval_fn=None):
    ...
    def max_value(state, alpha, beta, depth):
        ...
        # Maximize over all possible actions with pruning
        v = float('-inf')
        for action in game.actions(state):
            v = max(v, min_value(game.result(state, action), alpha, beta, depth - 1))
            if v >= beta:
                return v  # Beta cutoff - parent will ignore this branch
    ...
    def min_value(state, alpha, beta, depth):
        ...
        # Minimize over all possible actions with pruning
        v = float('inf')
        for action in game.actions(state):
            v = min(v, max_value(game.result(state, action), alpha, beta, depth - 1))
            if v <= alpha:
                return v  # Alpha cutoff - parent will ignore this branch
    ...
    for action in game.actions(state):
        ...
        if value > best_value:
            best_value = value
            best_action = action
        alpha = max(alpha, value)  # Update alpha at root level
