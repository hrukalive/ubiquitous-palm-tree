...
def minimax_cutoff_search(game, state, d=3, eval_fn=None):
    ...
    # Root logic: Choose the action that leads to the highest min_value
    best_score = -float('inf')
    best_action = None

    for action in game.actions(state):
        score = min_value(game.result(state, action), 1)
        if score > best_score:
            best_score = score
            best_action = action
...
def alpha_beta_cutoff_search(game, state, d=4, eval_fn=None):
    ...
    for action in game.actions(state):
        score = min_value(game.result(state, action), 1, alpha, beta)
        if score > best_score:
            best_score = score
            best_action = action
