...
from breakthrough import ag_eval_fn, offensive_eval_2
...
def main():
    ...
    for _ in range(5):  # Play each matchup 5 times
        ...
        b_agent = AlphaBetaAgent("Alphabeta", 2, eval_fn=offensive_eval_2)
        res = play_game(w_agent, b_agent, max_moves=400, progress=True)
        if res["winner"] == "black":
            win_count += 1
    for _ in range(5):  # Play each matchup 5 times
        w_agent = AlphaBetaAgent("Alphabeta", 2, eval_fn=offensive_eval_2)
