...
def minimax_cutoff_search(game, state, d=3, eval_fn=None):
    ...
    def max_value(state, depth):
        ...
        for action in game.actions(state):
            temp = max(temp, min_value(game.result(state, action), depth - 1))
        return temp
    ...
    def min_value(state, depth):
        ...
        temp = float("inf")
        for action in game.actions(state):
            temp = min(temp, max_value(game.result(state, action), depth - 1))
        return temp
...
def alpha_beta_cutoff_search(game, state, d=4, eval_fn=None):
    ...
    def max_value(state, alpha, beta, depth):
        ...
        temp = float("-inf")
        for action in game.actions(state):
            temp = max(temp, min_value(game.result(state, action), alpha, beta, depth + 1))
    ...
    def min_value(state, alpha, beta, depth):
        ...
        for action in game.actions(state):
            temp = min(temp, max_value(game.result(state, action), alpha, beta, depth + 1))
    ...
    for action in game.actions(state):
        value = min_value(game.result(state, action), best_value, beta, 1)
        if value > best_value:
            best_value = value
            best_action = action
