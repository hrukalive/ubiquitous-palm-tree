...
def minimax_cutoff_search(game, state, d=3, eval_fn=None):
    ...
    def max_value(state, depth):
        ...
        for action in game.actions(state):
            value2= min_value(game.result(state, action), depth - 1)
            if value2 > value:
                value = value2
    ...
    def min_value(state, depth):
        ...
        value = float('inf')
        for action in game.actions(state):
            value2 = max_value(game.result(state, action), depth - 1)
            if value2 < value:
                value = value2
...
def alpha_beta_cutoff_search(game, state, d=4, eval_fn=None):
    ...
    def max_value(state, depth, alpha, beta):
        ...
        value = float('-inf')
        for action in game.actions(state):
            value = max(value, min_value(game.result(state, action), depth - 1, alpha, beta))
            if value >= beta:
                return value
    ...
    def min_value(state, depth, alpha, beta):
        ...
        value = float('inf')
        for action in game.actions(state):
            value = min(value, max_value(game.result(state, action), depth - 1, alpha, beta))
            if value <= alpha:
                return value
            beta = min(beta, value)
    ...
    for action in game.actions(state):
        value = max_value(game.result(state, action), d, alpha, beta)
        if value > best_score:
            best_score = value
            best_action = action
