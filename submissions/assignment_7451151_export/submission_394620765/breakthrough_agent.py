...
def minimax_cutoff_search(game, state, d=3, eval_fn=None):
    ...
    def evaluate(s):
        return eval_fn(s, player) if eval_fn is not None else game.utility(s, player)
...
def alpha_beta_cutoff_search(game, state, d=4, eval_fn=None):
    ...
    def evaluate(state):
        return eval_fn(state, player) if eval_fn is not None else game.utility(state, player)
    ...
    for a in game.actions(state):
        expanded_nodes += 1
        value = min_value(game.result(state, a), alpha, beta, d -1)
        if value > best_score:
            best_score = value
            best_action = a
