...
class PriorityQueue:
    ...
    def __contains__(self, key):
        ...
        return key in self._elem_idxs

    def __getitem__(self, key):
        idx = self._get_elem_idxs(elem_id=key)
        if idx is not None:
    ...
    def _swap_elems(self, i, j):
        elem1, elem2 = self._heap[i], self._heap[j]
    ...
    def _bubble_up(self, i):
        while True:
            ...
            if not self.has_higher_priority(self._heap[i], self._heap[parent_idx]):
                return
        ...
        while True:
            ...
            if right_child_idx < len(self._heap):
                if self.has_higher_priority(
                    self._heap[right_child_idx], self._heap[prio_idx]
                ):
                    prio_idx = right_child_idx

            if not self.has_higher_priority(self._heap[prio_idx], self._heap[i]):
                return
    ...
    def _heapify(self):
        if len(self._heap) == 0:
            return
        max_idx = self.parent_idx(len(self._heap) - 1)
        for idx in range(max_idx, -1, -1):
            self._bubble_down(idx)

    def _append(self, elem):
        self._add_elem_idx(elem=elem, idx=len(self._heap))
    ...
    def pop(self):
        if len(self._heap) == 0:
            return None
        self._swap_elems(0, len(self._heap) - 1)
